...
var ImageCreator = function () {
  function ImageCreator(page, options) {
    ...
    this.context = wx.createCanvasContext(options.canvasId, page);
  ...
  _createClass(ImageCreator, [{
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                _context.next = 3;
                return _this.drawImage({ path: path, x: 0, y: 0, width: width, height: height });
              ...
              case 5:
                _context.next = 7;
                return _this.drawRect({ x: 0, y: 0, width: width, height: height });

              case 7:
                _this.drawComplateCount++;
                _this.canvasGenreated = true;
                _this.drawMainContent();
                    ...
                    while (1) {
                      switch (_context2.prev = _context2.next) {
                        case 0:
                          ...
                          _context2.next = 6;
                          return _this2.downloadPromise(options[i].url).then(function (res) {
                        ...
                        case 11:
                          if (options[i].url.match(/(http:\/\/|https:\/\/)/)) {
                            _this2.downloadPromise(options[i].url).then(function (res) {
                              options[i].path = res.tempFilePath;
                            });
                          } else {
                            options[i].path = options[i].url;
      ...
      if (imgArr.length) {
        ...
        var _loop2 = function _loop2(i, j) {
          if (imgArr[i].url && imgArr[i].url.match(/(http:\/\/|https:\/\/)/)) {
            _this3.formatImgsInfoArr[i] = _this3.createImageDownloadPromise(imgArr[i]);
          } else {
            _this3.formatImgsInfoArr[i] = new Promise(function (resolve, reject) {
              _this3.setImageInfo(imgArr[i]).then(function (res) {
                resolve(res);
    ...
    value: function getAllImgInfo() {
      ...
      Promise.all(this.formatImgsInfoArr).then(function (arr) {
        ...
        console.log('arr', arr)
        _this4.canDraw = true;
        _this4.formatImgsInfoArr = arr.slice(0);
        _this4.initCanvas();
      }).catch(function (e) {
    ...
    value: function createImageDownloadPromise(imgItemObj) {
      ...
      return new Promise(function (resolve, reject) {
        _this7.downloadPromise(imgItemObj.url).then(function (res) {
          var imgInfoObj = _extends({}, imgItemObj, {
            url: res.tempFilePath,
            originUrl: imgItemObj.url
          });
          _this7.setImageInfo(imgInfoObj).then(function (res) {
    ...
    value: function generateImage() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var generateTimeout = function generateTimeout() {
          setTimeout(function () {
            wx.canvasToTempFilePath({
              canvasId: _this8.canvas,
              y: 1,
              fileType: _this8.fileType || 'png',
              success: function success(res) {
                _this8.preview && wx.previewImage({
        ...
        _this8.context.draw(true, generateTimeout());
      });
    }
    /**
    ...
    value: function draw() {
      ...
      return new Promise(function (resolve, reject) {
        _this9.timeoutHandler = setTimeout(function () {
          _this9.clearTimeout();
          wx.showToast({title: "生成失败，请重试"});
          !_this9.hasDrawComplete() && console.warn('网络超时，请重试') && reject('timeout');
        }, _this9.options.timeout || 5000);

        var generateImage = function generateImage() {
          _this9.generateImage().then(function (res) {
            resolve(res);
          });
        };
        var statusObserve = function statusObserve() {
          _this9.statusObserveID = setTimeout(function () {
            if (_this9.hasDrawComplete()) {
              generateImage();
              _this9.clearTimeout();
            } else {
              statusObserve();
    ...
    value: function drawImage(content) {
      ...
      return new Promise(function (resolve) {
        ...
        _this10.context.rotate(rotate * Math.PI / 180);
        if (radius) {
          _this10.context.save();
          _this10.context.beginPath();
          _this10.context.lineTo(x + width - radius, y);
          _this10.context.arc(x + width - radius, y + radius, radius, 1.5 * Math.PI, 2 * Math.PI, false);
          _this10.context.lineTo(x + width, y + height - radius);
          _this10.context.arc(x + width - radius, y + height - radius, radius, 0, 0.5 * Math.PI, false);
          _this10.context.lineTo(x + radius, y + height);
          _this10.context.arc(x + radius, y + height - radius, radius, 0.5 * Math.PI, Math.PI, false);
          _this10.context.lineTo(x, y + radius);
          _this10.context.arc(x + radius, y + radius, radius, Math.PI, 1.5 * Math.PI, false);
          _this10.context.clip();

          _this10.context.drawImage(path, x, y, width, height);
          _this10.context.draw(true, setTimeout(function () {
    ...
    value: function drawArcImage(content) {
      ...
      return new Promise(function (resolve) {
        ...
        _this11.context.setLineWidth(borderWidth);
        _this11.context.arc(x, y, radius, 0, 2 * Math.PI);
        _this11.context.setStrokeStyle(borderColor);
        _this11.context.stroke();
        _this11.context.clip();
        _this11.context.drawImage(path, x - radius, y - radius, 2 * radius, 2 * radius);
    ...
    value: function drawRect(content) {
      ...
      return new Promise(function (resolve) {
        ...
        if (shadow) {
          _this12.context.shadowOffsetX = shadow.offsetX || 0;
          _this12.context.shadowOffsetY = shadow.offsetY || 0;
          _this12.context.shadowBlur = shadow.blur || 0;
          _this12.context.shadowColor = shadow.color || 'transparent';
        }
        _this12.context.setFillStyle(backgroundColor);
        _this12.context.beginPath();
        _this12.context.moveTo(x + (radius || topLeftRadius || 0), y);
        radius || topRightRadius ? _this12.context.lineTo(x + width - (radius || topRightRadius), y) : _this12.context.lineTo(x + width, y);
        (radius || topRightRadius) && _this12.context.arc(x + width - (radius || topRightRadius), y + (radius || topRightRadius), radius || topRightRadius, 1.5 * Math.PI, 2 * Math.PI, false);
        radius || bottomRightRadius ? _this12.context.lineTo(x + width, y + height - (radius || bottomRightRadius)) : _this12.context.lineTo(x + width, y + height);
        (radius || bottomRightRadius) && _this12.context.arc(x + width - (radius || bottomRightRadius), y + height - (radius || bottomRightRadius), radius || bottomRightRadius, 0, 0.5 * Math.PI, false);
        radius || bottomLeftRadius ? _this12.context.lineTo(x + (radius || bottomLeftRadius), y + height) : _this12.context.lineTo(x, y + height);
        (radius || bottomLeftRadius) && _this12.context.arc(x + (radius || bottomLeftRadius), y + height - (radius || bottomLeftRadius), radius || bottomLeftRadius, 0.5 * Math.PI, Math.PI, false);
        radius || topLeftRadius ? _this12.context.lineTo(x, y + (radius || topLeftRadius)) : _this12.context.lineTo(x, y);
        (radius || topLeftRadius) && _this12.context.arc(x + (radius || topLeftRadius), y + (radius || topLeftRadius), radius || topLeftRadius, Math.PI, 1.5 * Math.PI, false);
        // 增加填充渐变
        if (gradient) {
          ...
          if (type === 'linear') {
            var _options$startX = options.startX,
                ...
                endX = _options$endX === undefined ? _this12.canvasWidth : _options$endX,
                _options$endY = options.endY,
                endY = _options$endY === undefined ? _this12.canvasHeight : _options$endY;

            fillStyle = _this12.context.createLinearGradient(startX, startY, endX, endY);
            // 径向渐变
          } else if (type === 'radial') {
            var _options$startX2 = options.startX,
                ...
                _endX = _options$endX2 === undefined ? _this12.canvasWidth : _options$endX2,
                _options$endY2 = options.endY,
                _endY = _options$endY2 === undefined ? _this12.canvasHeight : _options$endY2,
                _options$endRadius = options.endRadius,
                endRadius = _options$endRadius === undefined ? 0 : _options$endRadius;

            fillStyle = _this12.context.createRadialGradient(_startX, _startY, startRadius, _endX, _endY, endRadius);
          }
          ...
          _this12.context.setFillStyle(fillStyle);
        }

        _this12.context.fill();
        _this12.context.setLineWidth(lineWidth);
        _this12.context.setStrokeStyle(borderColor);
        _this12.context.stroke();
        _this12.context.shadowOffsetX = 0;
        _this12.context.shadowOffsetY = 0;
        _this12.context.shadowBlur = 0;
        _this12.context.shadowColor = 'transparent';
    ...
    value: function drawCircle(content) {
      ...
      return new Promise(function (resolve) {
        ...
        _this13.context.lineWidth = borderWidth;
        borderStyle == 'dashed' && _this13.context.setLineDash([dashedWidth, dashedOffset]);
        _this13.context.arc(x, y, radius, 0, Math.PI * 2);
    ...
    value: function drawLine(content) {
      ...
      return new Promise(function (resolve) {
        ...
        _this14.context.moveTo(x, y);
        _this14.context.lineWidth = type == "vertical" ? width : height;
        lineStyle == 'dashed' && _this14.context.setLineDash([type == "vertical" ? dashedHeight : dashedWidth, dashedOffset]);
        _this14.context.setStrokeStyle(color);
        _this14.context.lineTo(type == "vertical" ? x : x + width, type == "vertical" ? y + height : y);
    ...
    value: function drawText(item) {
      ...
      return new Promise(function (resolve) {
        ...
        if (setFontWeight) {
          ...
          _this15.context.setLineWidth(outLineWidth);
          _this15.context.setStrokeStyle(color);
        }
        _this15.context.setTextAlign(textAlign);
        _this15.context.setFontSize(fontSize);
        _this15.context.setFillStyle(color);
        _this15.context.setTextBaseline('top');
        if (/(.*\$\{(.*)\}\$)/.test(text)) {
        ...
        if (padding) {
          _this15.drawTextWidthPadding(_this15.canvasWidth - padding * 2, item, text, textAlign == 'center' ? _this15.canvasWidth / 2 : textAlign == 'right' ? _this15.canvasWidth - padding : padding);
        } else if (paddingLeft && !paddingRight) {
          _this15.drawTextWidthPadding(_this15.canvasWidth - paddingLeft, item, text, paddingLeft);
        } else if (!paddingLeft && paddingRight) {
          _this15.drawTextWidthPadding(_this15.canvasWidth - paddingRight, item, text, x);
        } else if (paddingLeft && paddingRight) {
          _this15.drawTextWidthPadding(_this15.canvasWidth - paddingRight - paddingLeft, item, text, paddingLeft);
        } else if (width > 0) {
          _this15.drawTextWithWidth(width, item, text, x);
        } else {
          if (setFontWeight) {
            _this15.context.fillText(text, x, y);
            _this15.context.fillText(text, x, y + outLineWidth);
            _this15.context.fillText(text, x, y - outLineWidth);
            _this15.context.fillText(text, x + outLineWidth, y);
            _this15.context.fillText(text, x - outLineWidth, y);
          } else {
            _this15.context.fillText(text, x, y);
          }
    ...
    value: function drawTextWidthLetterSpacing(text, x, y, letterSpacing) {
      ...
      arrText.forEach(function (letter) {
        var letterWidth = _this16.context.measureText(letter).width;
        _this16.context.fillText(letter, x, y);
        // 确定下一个字符的横坐标
    ...
    value: function hasDrawComplete() {
      ...
      console.log('this.renderElementCount', this.renderElementCount)
